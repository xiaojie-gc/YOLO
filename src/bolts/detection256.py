...
class DetectionBolt_256(Bolt):
    ...
    def process(self, tup):
        ...
        img_name = str(self.total) + "_256_" + img_name
        self.logger.info("start to process image = {}".format(img_name))
        self._increment()
        img_path = "/home/ubuntu/data/inputs"
        with open(img_path + "/" + img_name, 'wb') as file:
            file.write(base64.b64decode(img_data))
        objs, cpu, computation_time = self.detect_image(img_name,  img_path)
        self.emit(["YOLO256", [cpu, computation_time], self.total, objs])
